...
public final class MimeTypes implements Detector, Serializable {
    private static final Map CLASSLOADER_SPECIFIC_DEFAULT_TYPES = new HashMap();
    ...
    private List applyHint(List list, MimeType mimeType) {
    ...
    */
    ...
    public MimeType forName(String str) throws MimeTypeException {
        ...
        MimeType mimeType = (MimeType) this.types.get(normalize);
        if (mimeType == null) {
            synchronized (this) {
                try {
                    MimeType mimeType2 = (MimeType) this.types.get(normalize);
                    if (mimeType2 == null) {
    ...
    public MimeType getRegisteredMimeType(String str) throws MimeTypeException {
        ...
        if (normalize.hasParameters()) {
            return (MimeType) this.types.get(normalize.getBaseType());
        }
        return null;
    }
    ...
    public static synchronized MimeTypes getDefaultMimeTypes(ClassLoader classLoader) {
        ...
        synchronized (MimeTypes.class) {
            ...
            if (classLoader != null) {
                mimeTypes = (MimeTypes) CLASSLOADER_SPECIFIC_DEFAULT_TYPES.get(classLoader);
            }
            if (mimeTypes == null) {
                try {
                    try {
    ...
    public List getMimeType(byte[] bArr) {
        if (bArr != null) {
            if (bArr.length == 0) {
            ...
            if (!arrayList.isEmpty()) {
                for (int i2 = 0; i2 < arrayList.size(); i2++) {
                    ...
                    if ("application/xml".equals(mimeType.getName()) || "text/html".equals(mimeType.getName())) {
                        ...
                        if (extractRootElement != null) {
                            ...
                            while (true) {
                                if (it.hasNext()) {
                                    MimeType mimeType2 = (MimeType) it.next();
                                    if (mimeType2.matchesXML(extractRootElement.getNamespaceURI(), extractRootElement.getLocalPart())) {
                                        arrayList.set(i2, mimeType2);
                                        break;
                                    }
                                }
                            }
                        } else if ("application/xml".equals(mimeType.getName())) {
                            Iterator it2 = this.magics.iterator();
                            while (true) {
                                if (it2.hasNext()) {
                                    Magic magic2 = (Magic) it2.next();
                                    if (magic2.getType().equals(this.htmlMimeType) && magic2.eval(bArr)) {
            ...
            try {
                return Collections.singletonList(forName(new TextDetector(getMinLength()).detect(h.a().q(bArr).p(), new Metadata()).toString()));
