...
public class EmbeddedDocumentUtil implements Serializable {
    ...
    private static Parser findInComposite(CompositeParser compositeParser, Class cls, ParseContext parseContext) {
        for (Parser parser : compositeParser.getAllComponentParsers()) {
            ...
            if (parser instanceof CompositeParser) {
                parser = findInComposite((CompositeParser) parser, cls, parseContext);
            }
            if (equals(parser, cls)) {
                return parser;
            }
    ...
    private static Parser findInDecorated(ParserDecorator parserDecorator, Class cls) {
        ...
        return equals(wrappedParser, cls) ? wrappedParser : wrappedParser instanceof ParserDecorator ? findInDecorated((ParserDecorator) wrappedParser, cls) : wrappedParser;
    }
    ...
    */
    ...
    public String getExtension(TikaInputStream tikaInputStream, Metadata metadata) {
        ...
        if (forName == null) {
            try {
                forName = mimeTypes.forName(getDetector().detect(tikaInputStream, metadata).toString());
                z = true;
