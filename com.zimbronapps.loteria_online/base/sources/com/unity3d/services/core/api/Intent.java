...
public class Intent {
    ...
    public static void canOpenIntents(JSONArray jSONArray, WebViewCallback webViewCallback) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            try {
                ...
                webViewCallback.error(IntentError.JSON_EXCEPTION, e.getMessage());
                return;
    ...
    public static void checkProviders(String str, WebViewCallback webViewCallback) {
        ...
        List queryIntentContentProviders = ClientProperties.getApplicationContext().getPackageManager().queryIntentContentProviders(new android.content.Intent(str), 0);
        if (queryIntentContentProviders != null && queryIntentContentProviders.size() == 0) {
            webViewCallback.invoke(arrayList);
            return;
        }
        Iterator it = queryIntentContentProviders.iterator();
        while (it.hasNext()) {
            arrayList.add(((ResolveInfo) it.next()).providerInfo.authority);
        }
        webViewCallback.invoke(arrayList);
    }
