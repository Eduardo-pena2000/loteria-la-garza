...
public class StorageManager {
    private static final Map _storageFileMap = Collections.synchronizedMap(new HashMap());
    private static final List _storages = Collections.synchronizedList(new ArrayList());
    ...
    public static boolean hasStorage(StorageType storageType) {
        ...
        synchronized (list) {
            try {
                ...
                while (it.hasNext()) {
                    if (((Storage) it.next()).getType().equals(storageType)) {
                        return true;
                    }
                }
                return false;
