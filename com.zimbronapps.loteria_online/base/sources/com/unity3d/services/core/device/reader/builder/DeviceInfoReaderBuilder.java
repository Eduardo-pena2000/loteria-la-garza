...
public class DeviceInfoReaderBuilder {
    ...
    private JsonFlattenerRules getTsiRequestStorageRules() {
        return new JsonFlattenerRules(Arrays.asList(new String[]{"privacy", "gdpr", "framework", "adapter", "unity", "pipl", "configuration", "user", "unifiedconfig"}), Collections.singletonList("value"), Arrays.asList(new String[]{"ts", "exclude", "pii", "nonBehavioral", "nonbehavioral"}));
    }
    ...
    public JsonFlattenerRules getMediationInfoRules() {
        return new JsonFlattenerRules(Collections.singletonList("mediation"), Collections.singletonList("value"), Arrays.asList(new String[]{"ts", "traits"}));
    }
}
