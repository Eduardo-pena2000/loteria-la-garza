package com.revenuecat.purchases.google.attribution;

import Da.D;
import Qa.l;
import com.revenuecat.purchases.common.LogIntent;
import java.io.IOException;
import java.util.Arrays;
import kotlin.jvm.internal.t;
import kotlin.jvm.internal.u;

/* loaded from: /storage/emulated/0/Android/data/com.apktools.app.decompile/files/decompile_temp/jadx/classes4.dex */
public final class GoogleDeviceIdentifiersFetcher$getAdvertisingID$$inlined$log$5 extends u implements Qa.a {
    final /* synthetic */ IOException $e$inlined;
    final /* synthetic */ LogIntent $intent;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public GoogleDeviceIdentifiersFetcher$getAdvertisingID$$inlined$log$5(LogIntent logIntent, IOException iOException) {
        super(0);
        this.$intent = logIntent;
        this.$e$inlined = iOException;
    }

    public final String invoke() {
        StringBuilder sb = new StringBuilder();
        sb.append(D.n0(this.$intent.getEmojiList(), "", (CharSequence) null, (CharSequence) null, 0, (CharSequence) null, (l) null, 62, (Object) null));
        sb.append(' ');
        String format = String.format("IOException when getting advertising identifier. Message: %s", Arrays.copyOf(new Object[]{this.$e$inlined.getLocalizedMessage()}, 1));
        t.f(format, "format(...)");
        sb.append(format);
        return sb.toString();
    }
}
