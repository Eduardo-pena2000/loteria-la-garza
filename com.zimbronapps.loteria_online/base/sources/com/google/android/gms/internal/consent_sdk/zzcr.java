...
public final class zzcr {
    ...
    final AtomicReference zza = new AtomicReference((Object) null);
    ...
    public static /* synthetic */ void zza(zzcr zzcrVar) {
        zzcrVar.zzl();
    }
    ...
    private final void zzl() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (zzifVar != null) {
                try {
                    HttpURLConnection openConnection = new URI(str).toURL().openConnection();
                    openConnection.setRequestProperty("User-Agent", WebSettings.getDefaultUserAgent(this.zzd));
                    openConnection.setConnectTimeout(10000);
                    openConnection.setReadTimeout(30000);
                    openConnection.setDoOutput(true);
                    openConnection.setRequestMethod("POST");
                    openConnection.setRequestProperty("Content-Type", "application/protobuf");
                    zzifVar.zzE(openConnection.getOutputStream());
                    int responseCode = openConnection.getResponseCode();
                    if (responseCode != 200 && responseCode != 204) {
                        Log.w("UserMessagingPlatform", "Fail to ping metrics reporting URL: Http error code - " + responseCode + ".\n" + (openConnection.getErrorStream() != null ? new Scanner(openConnection.getErrorStream()).useDelimiter("\\A").next() : null));
                        ((Queue) atomicReference.get()).add(zzifVar);
                    }
                } catch (URISyntaxException e) {
                    Log.w("UserMessagingPlatform", "Metrics reporting URL " + str + " is not valid: " + e.getMessage());
    ...
    private final void zzm(Queue queue, int i, int i2, String str) {
        ...
        if (l != null) {
            ...
            this.zzf.execute(new zzcq(this));
        }
    }
