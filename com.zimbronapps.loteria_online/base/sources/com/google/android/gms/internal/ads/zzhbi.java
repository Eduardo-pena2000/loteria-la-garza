package com.google.android.gms.internal.ads;

/* loaded from: /storage/emulated/0/Android/data/com.apktools.app.decompile/files/decompile_temp/jadx/classes3.dex */
public final class zzhbi {
    private static final zzhbg zza = zzhbh.zza;
    private final zzhaz zzb;
    private final zzhbb zzc;
    private final int zzd;
    private final boolean zze;
    private final boolean zzf;
    private final zzhbg zzg;
    private final int zzh;

    public /* synthetic */ zzhbi(zzhaz zzhazVar, int i, int i2, boolean z, boolean z2, zzhbg zzhbgVar, byte[] bArr) {
        this.zzb = zzhazVar;
        this.zzh = i;
        int i3 = i - 2;
        this.zzc = i3 != 1 ? i3 != 3 ? zzhbb.zzb : zzhbb.zzc : zzhbb.zza;
        this.zzd = i2;
        this.zze = z;
        this.zzf = z2;
        this.zzg = zzhbgVar;
    }

    public static /* synthetic */ zzhbg zze() {
        return zza;
    }

    public final zzhaz zza() {
        this.zzg.zza(this);
        return this.zzb;
    }

    public final zzhbb zzb() {
        return this.zzc;
    }

    public final int zzc() {
        return this.zzd;
    }

    public final boolean zzd() {
        return this.zze;
    }

    public final /* synthetic */ zzhaz zzf() {
        return this.zzb;
    }

    public final /* synthetic */ int zzg() {
        return this.zzd;
    }

    public final /* synthetic */ boolean zzh() {
        return this.zze;
    }

    public final /* synthetic */ boolean zzi() {
        return this.zzf;
    }

    public final /* synthetic */ int zzj() {
        return this.zzh;
    }
}
