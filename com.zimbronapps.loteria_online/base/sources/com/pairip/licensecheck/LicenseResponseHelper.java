...
public class LicenseResponseHelper {
    ...
    public static RepeatedCheckMetadata getRepeatedCheckMetadata(Bundle responsePayload) throws LicenseCheckException {
        try {
            ...
            if (base64ToJson.has("repeatedCheckDurationToRetryMillis") && base64ToJson.has("repeatedCheckTimeToRetryMillis")) {
                return new RepeatedCheckMetadata(base64ToJson.getLong("repeatedCheckDurationToRetryMillis"), base64ToJson.getLong("repeatedCheckTimeToRetryMillis"));
            }
            return null;
        } catch (JSONException e) {
            throw new LicenseCheckException("Could not decode json", e);
    ...
    private static String[] getJwsPartsForLicenseData(Bundle responsePayload) throws LicenseCheckException {
        String string = responsePayload.getString("LICENSE_DATA");
        if (string == null) {
    ...
    private static void verifySignature(String signedData, String signature, String signatureAlgorithm, PublicKey publicKey) throws LicenseCheckException {
        try {
            ...
            throw new LicenseCheckException(String.format("Could not find %s algorithm on the device", new Object[]{signatureAlgorithm}), e);
    ...
    private static PublicKey getPublicKey() throws LicenseCheckException {
        try {
            ...
            throw new LicenseCheckException("Could not create key specification from the public key", e);
        } catch (IllegalArgumentException e2) {
            throw new LicenseCheckException("Could not decode public key", e2);
        } catch (NoSuchAlgorithmException e3) {
            throw new LicenseCheckException(String.format("%s algorithm not found on device", new Object[]{"RSA"}), e3);
        }
    }
